...
| `jsonPath` | string | No | `$.messages[-1].content` | JSONPath expression to extract a specific value from JSON payload. Use `""` to process the entire payload as a string. |
...
If `jsonPath` is not specified, `$.messages[-1].content` is used. Set `jsonPath: ""` to process the entire payload as a string.
